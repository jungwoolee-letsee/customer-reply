...
function createSmokePipeline(_track) {
  ...
  const stripMat = _track(new THREE.MeshBasicMaterial({
    map: texture,
    opacity: 0.5,
  ...
  const stripMatUp = _track(new THREE.MeshBasicMaterial({
    map: textureUp,
    opacity: 0.5,
    side: THREE.DoubleSide,
    depthWrite: false,
  ...
  let material = _track(new THREE.MeshBasicMaterial({
    // color: 0x00aee4,
    opacity: 1,
